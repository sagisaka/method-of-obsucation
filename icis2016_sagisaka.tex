...
%%
%AllatoriとProGuardで同じ
%5-gramが追加されている場合、異なる5-gramが追加されている場合がある。追
%加された5-gramは最大で1,000程度存在することもわかる。
%
%一方で、yGuardのヒストグラムは、表\ref{table:}からもわかるように、
...
%つの命令列から取り出した$5$-gramは、オリジナルの$5$-gramにも含まれているこ
%とが確認できた。
%
%このことから、Allatoriは命令の並び中に \texttt{swap} 命令を入れ、命令
%の順序を入れ替えることで難読化を実現していると考えられる。Allatoriのみ
...
\begin{figure}[b]
  \centering
  \subfigure[オリジナル]{
    \includegraphics[clip,width=0.48\columnwidth]{images/ORI.eps}
    \label{fig:asm-5gram-original-histogram}
  }%
  \subfigure[ALL]{
    \includegraphics[clip,width=0.48\columnwidth]{images/ALL.eps}
    \label{fig:asm-5gram-ALL-histogram}
  }
  \subfigure[DR]{
    \includegraphics[clip,width=0.48\columnwidth]{images/DR.eps}
    \label{fig:asm-5gram-DR-histogram}
  }%
  \subfigure[IRR]{
    \includegraphics[clip,width=0.48\columnwidth]{images/IRR.eps}
    \label{fig:asm-5gram-IRR-histogram}
  }
  \subfigure[MLI]{
    \includegraphics[clip,width=0.48\columnwidth]{images/MLI.eps}
    \label{fig:asm-5gram-MLI-histogram}
  }%
  \subfigure[PG]{
    \includegraphics[clip,width=0.48\columnwidth]{images/PG.eps}
